...
def iterate_dataframes_path(
    path: str, supress_tqdm=False
...
def aggregate_data(
    dir_path: str, target_path: str, aggregate_function: Callable[[pd.DataFrame], dict]
) -> pd.DataFrame:
    """
...
def aggregate_data_path(
    dir_path: str,
    target_path: str,
    aggregate_function: Callable[[pd.DataFrame, str], dict],
) -> pd.DataFrame:
    """
    Creates aggregated data frame and saves it as csv.
    """
...
def aggregate_data_multiple_rows(
    dir_path: str,
    target_path: str,
    aggregate_function: Callable[[pd.DataFrame], list[dict]],
) -> pd.DataFrame:
    """
    Creates aggregated data frame and saves it as csv.
    """
...
def load_samples_with_numpy(
    path_to_directory: str, loading_func: Callable[[str], pd.DataFrame] = load_samples
) -> pd.DataFrame:
    """
    ...
    if (
        len([file for file in os.listdir(path_to_directory) if file.endswith(".csv")])
        != 1
    ):
        raise OSError("Too many csv files in directory.")

    df = loading_func(
        f"{path_to_directory}/{[file for file in os.listdir(path_to_directory) if file.endswith('.csv')][0]}"
    )

    for numpy_file in [
        file for file in os.listdir(path_to_directory) if file.endswith(".npy")
    ]:
...
def load_samples_with_list(
    cols_to_listify: list[str],
    ...
    for col in cols_to_listify:
        ...
        try:
            ...
            raise Exception(e, f"Error on col: {col}")
    return df
...
def series_numpy_equals(a: pd.Series, b: pd.Series) -> bool:
    ...
    return (
        ...
        .reindex()
        .apply(lambda x: np.array_equal(x["emb_1"], x["emb_2"]), axis=1)
        .all()
    )


def save_csv_with_embeddings(
    path_to_dir: str,
    df: pd.DataFrame,
