...
def exploration_in_numbers(df_agg: pd.DataFrame) -> None:
    ...
    print(f"First date: {df_agg[MetaDataSchema.DATE].min()}.  Last date: {df_agg[MetaDataSchema.DATE].max()}. Unique dates: {df_agg[MetaDataSchema.DATE].unique().shape[0]}.")
    print(f"Tweets duplicated or retweeted on the same day total: {df_agg[MetaDataSchema.DUPLICATED].sum()}. Tweets duplicated on the same day percent: {df_agg[MetaDataSchema.DUPLICATED].sum()/df_agg[MetaDataSchema.ROWS].sum() :.2%}" )
    print(f"Tweets not duplicated or retweeted on the same day total: {df_agg[MetaDataSchema.ROWS].sum()-df_agg[MetaDataSchema.DUPLICATED].sum()}")
    print(f"Tweets in a single file on average: {df_agg[MetaDataSchema.ROWS].mean()}, with variance: {df_agg[MetaDataSchema.ROWS].var()} and max: {df_agg[MetaDataSchema.ROWS].max()} and min: {df_agg[MetaDataSchema.ROWS].min()} ")  


def add_column_others(df: pd.DataFrame, languages_to_show:np.ndarray[str]) -> pd.DataFrame:
    """
    ...
    df['others'] = df.apply(lambda x: sum([x[other] for other in not_selected_languages]), axis = 1)
    df['others_dupl'] = df.apply(lambda x: sum([x[other+"_dupl"] for other in not_selected_languages]), axis = 1)
...
def languages_tabular(df: pd.DataFrame, languages_to_show: np.ndarray[str]) -> None:
    ...
    languages_to_show = np.append( languages_to_show,'others') if not languages_to_show.shape == (66,) else languages_to_show
    ...
    for language, count, freq, dupl in zip(languages_to_show, language_counts, language_freqs, language_dupl):
...
def languages_bar_h(df_agg: pd.DataFrame, languages_to_show:np.ndarray[str]) -> None:
    ...
    for i, la in enumerate(languages_to_show):
        ax.text((language_counts + language_dupl +800000*np.ones(language_dupl.shape[0]))[i], i,f"{language_freqs[i]:10.2%}", ha='center', va='center')
        ax.text((language_counts)[i], i,f"{(int(language_counts[i]))} ", ha='center', va='center')
...
def pi_lang_freq(df: pd.DataFrame, languages: np.ndarray[str], save_path: str=None, not_dupl: bool=False) -> None:
    ...
    ax.set_title(f'Percentage of {"not duplicated" if not_dupl else ""} tweets by language')
    plt.show()


def stackplot_languages_over_weeks(df: pd.DataFrame, selected_language: np.ndarray[str], save_path: str = None) -> None:
    """
    ...
    ax.stackplot(df_weeks.index, [df_weeks[la] for la in selected_language],
                 labels=selected_language, alpha=0.8)
...
def stackplot_duplicates_over_weeks(df: pd.DataFrame, save_path: str = None) -> None:
    ...
    df_weeks = df.groupby("weeks")[[MetaDataSchema.ROWS, MetaDataSchema.DUPLICATED]].sum()
    df_weeks[MetaDataSchema.ROWS] = df_weeks[MetaDataSchema.ROWS] - df_weeks[MetaDataSchema.DUPLICATED]
    
    
    
    fig, ax = plt.subplots()
    ax.stackplot(df_weeks.index, [df_weeks[MetaDataSchema.ROWS], df_weeks[MetaDataSchema.DUPLICATED]],
