...
def move_files_into_one_dir(archive_path: str) -> None:
    ...
    """
    if not os.path.isdir(archive_path + "/UkraineWar") or not os.path.isdir(
        archive_path + "/UkraineWar/UkraineWar"
    ):
        raise FileNotFoundError("Archive doesnt have expected structure")
    for file in os.listdir(archive_path + "/UkraineWar/UkraineWar"):
        os.rename(
            archive_path + "/UkraineWar/UkraineWar/" + file, archive_path + "/" + file
        )
...
def split_csv_by_date(data_path: str) -> None:
    ...
    for path in [
        ...
        if "to" in x and ".csv" in x
    ]:
        df = pd.read_csv(path)
        df["date"] = df["tweetcreatedts"].apply(lambda x: x[:10])
        dfs = {
            "".join(date.split("-"))
            + "_UkraineCombinedTweetsDeduped.csv": df[df["date"] == date]
...
def merge_splitted_files(data_path: str) -> None:
    ...
    if not data_path[-1] == "/":
        data_path = data_path + "/"
    if not "UkraineCombinedTweetsDeduped_FEB28_part2.csv" in os.listdir(
        data_path
    ) or not "UkraineCombinedTweetsDeduped_FEB28_part1.csv" in os.listdir(
        data_path
    ):  #
...
def print_folder_stats(data_path: str) -> None:
    ...
    print(
        f"First date: {sorted(os.listdir(data_path))[0]}, Last date: {sorted(os.listdir(data_path))[-1]}"
    )
    files = [
        os.stat(data_path + "/" + path).st_size / (1024 * 1024)
